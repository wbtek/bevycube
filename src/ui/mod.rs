...
pub fn spawn_settings_ui(
    ...
    let set_anisotropic_id = commands.spawn(( SetAnisotropic, Mesh3d(meshes.add(Plane3d::default().mesh().size(5./16., 5./16.))), MeshMaterial3d(diamond_mat.clone()), Transform::from_xyz(to_local(107.+14.), 0.01, to_local(140.+22.)) )).id();
    et.set_anisotropic = Some(set_anisotropic_id);
    commands.entity(settings_id).add_child(set_anisotropic_id);

    let set_mipmaps_id = commands.spawn(( SetMipmaps, Mesh3d(meshes.add(Plane3d::default().mesh().size(5./16., 5./16.))), MeshMaterial3d(diamond_mat.clone()), Transform::from_xyz(to_local(107.+14.), 0.01, to_local(230.+22.)) )).id();
    et.set_mipmaps = Some(set_mipmaps_id);
    commands.entity(settings_id).add_child(set_mipmaps_id);

    let set_resolution_id = commands.spawn(( SetResolution, Mesh3d(meshes.add(Plane3d::default().mesh().size(5./16., 5./16.))), MeshMaterial3d(diamond_mat.clone()), Transform::from_xyz(to_local(107.+14.), 0.01, to_local(320.+22.)) )).id();
    et.set_resolution = Some(set_resolution_id);
    commands.entity(settings_id).add_child(set_resolution_id);

    let set_fps_id = commands.spawn(( SetFps, Mesh3d(meshes.add(Plane3d::default().mesh().size(5./16., 5./16.))), MeshMaterial3d(diamond_mat.clone()), Transform::from_xyz(to_local(107.+14.), 0.01, to_local(410.+22.)) )).id();
